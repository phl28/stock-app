	...
	<div class="flex justify-end space-x-2">
		{#if selectedTrades.length > 0}
			...
			{#if selectedTickers.length === 1}
				<button class="btn btn-primary" onclick={toggleAssignTradeModal}>Assign To Position</button>
			{/if}
		{/if}
		<button class="btn btn-neutral" onclick={toggleAddTradeModal}>Add</button>
